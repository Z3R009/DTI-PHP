...
if (isset($_GET['oo3_1_id']) && $_GET['confirm'] == 'yes') {
    // Get the user ID from the query string
    $oo3_1_id = intval($_GET['oo3_1_id']);

    // Prepare and execute the deletion query for 'users' table
    $deleteUserSql = "DELETE FROM oo3_1_allotment WHERE oo3_1_id = ?";
    $stmtUser = $connection->prepare($deleteUserSql);
    $stmtUser->bind_param("i", $oo3_1_id);
