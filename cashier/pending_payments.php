                        ...
                        <div class="table-responsive">
                            ...
                            <div class="modal fade" id="batchAdaModal" tabindex="-1" aria-labelledby="batchAdaModalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-xl">
                                    <div class="modal-content">
                                        <form method="POST" action="back_end/batch_ada_payment.php" id="batchADAModalForm">
                                            <div class="modal-header bg-primary text-white">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div id="selectedDvsContainer">
                                                    <!-- Selected DVs will be added here as hidden inputs -->
                                                </div>
                                                <div id="selectedMergedGroupsContainer">
                                                    <!-- Selected merged groups will be added here as hidden inputs -->
                                                </div>
                                                <div id="adaReferencesContainer">
                                                    <!-- ADA references will be added here as hidden inputs -->
                                                </div>
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button type="submit" class="btn btn-primary" id="submitBatchAdaBtn" name="submit_batch_ada">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (batchAdaModal) {
        batchAdaModal.addEventListener('show.bs.modal', function() {
            ...
            const useCommonAdaInput = document.getElementById('use_common_ada');
            ...
            // Initialize use_common_ada hidden input value
            if (useCommonAdaInput) {
                const useCommonAda = document.getElementById('useCommonAda').checked;
                useCommonAdaInput.value = useCommonAda ? '1' : '0';
                console.log('Initialized use_common_ada value to:', useCommonAdaInput.value);
            }
    ...
    if (useCommonAdaRadio && useMultipleAdaRadio) {
        useCommonAdaRadio.addEventListener('change', function() {
            if (this.checked) {
                toggleAdaReferenceInputs(true);
                const useCommonAdaInput = document.getElementById('use_common_ada');
                if (useCommonAdaInput) {
                    useCommonAdaInput.value = '1';
                    console.log('Set use_common_ada value to 1');
                }
            }
        });
        ...
        useMultipleAdaRadio.addEventListener('change', function() {
            if (this.checked) {
                toggleAdaReferenceInputs(false);
                const useCommonAdaInput = document.getElementById('use_common_ada');
                if (useCommonAdaInput) {
                    useCommonAdaInput.value = '0';
                    console.log('Set use_common_ada value to 0');
                }
            }
        });
    }
    ...
    function toggleAdaReferenceInputs(useCommon = true) {
        ...
        // Update the use_common_ada hidden input
        if (useCommonAdaInput) {
            useCommonAdaInput.value = useCommon ? '1' : '0';
            console.log('Updated use_common_ada value to:', useCommonAdaInput.value);
        } else {
            console.error('use_common_ada input not found');
        }
    }
    ...
    if (submitBatchAdaBtn) {
        submitBatchAdaBtn.addEventListener('click', function(e) {
            e.preventDefault();
            console.log('Batch payment button clicked');
            ...
            showAlert({
                ...
                if (result.isConfirmed) {
                    ...
                    // Submit the form
                    console.log('Submitting batch payment form');
                    
                    // Add selected DVs and merged groups to form
                    const form = document.getElementById('batchADAModalForm');
                    
                    // Remove any existing selected_dvs[] inputs
                    form.querySelectorAll('input[name="selected_dvs[]"]').forEach(input => input.remove());
                    form.querySelectorAll('input[name="selected_merged_groups[]"]').forEach(input => input.remove());
                    form.querySelectorAll('input[name^="ada_references"]').forEach(input => input.remove());
                    
                    // Add selected DVs
                    selectedDVs.forEach(dv => {
                        const input = document.createElement('input');
                        input.type = 'hidden';
                        input.name = 'selected_dvs[]';
                        input.value = dv.value;
                        form.appendChild(input);
                        
                        // Add reference number if using individual references
                        if (!useCommonAda) {
                            const refInput = document.querySelector(`.ada-reference-input[data-dv-id="${dv.value}"]`);
                            if (refInput && refInput.value) {
                                const adaRefInput = document.createElement('input');
                                adaRefInput.type = 'hidden';
                                adaRefInput.name = `ada_references[dv_${dv.value}]`;
                                adaRefInput.value = refInput.value;
                                form.appendChild(adaRefInput);
                            }
                        }
                    });
                    ...
                    selectedGroups.forEach(group => {
                        ...
                        if (!useCommonAda) {
                            ...
                            if (refInput && refInput.value) {
                                const adaRefInput = document.createElement('input');
                                adaRefInput.type = 'hidden';
                                adaRefInput.name = `ada_references[merge_${group.value}]`;
                                adaRefInput.value = refInput.value;
                                form.appendChild(adaRefInput);
                            }
                        }
                    ...
                    // Add common ADA reference if using one
                    if (useCommonAda && commonAdaRef) {
                        const input = document.createElement('input');
                        input.type = 'hidden';
                        input.name = 'common_ada_ref';
                        input.value = commonAdaRef;
                        form.appendChild(input);
                        
                        // Also set the use_common_ada input 
                        const useCommonInput = document.getElementById('use_common_ada');
                        if (useCommonInput) {
                            useCommonInput.value = '1';
                        } else {
                            const hiddenInput = document.createElement('input');
                            hiddenInput.type = 'hidden';
                            hiddenInput.name = 'use_common_ada';
                            hiddenInput.value = '1';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const batchAdaForm = document.getElementById('batchADAModalForm');
    const submitBatchAdaBtn = document.getElementById('submitBatchAdaBtn');
    const accountSelect = document.getElementById('account_name');
    const fundCodeInput = document.getElementById('fund_code');
    const batchDateInput = document.getElementById('batch_date');
    const commonAdaRefInput = document.getElementById('common_ada_ref');
    const useCommonAdaRadio = document.getElementById('useCommonAda');
    const useMultipleAdaRadio = document.getElementById('useMultipleAda');
    const commonAdaSection = document.getElementById('commonAdaSection');

    // Enable the submit button by default
    if (submitBatchAdaBtn) {
        submitBatchAdaBtn.disabled = false;
    }
    ...
    // Update ADA reference section visibility
    if (useCommonAdaRadio && useMultipleAdaRadio) {
        useCommonAdaRadio.addEventListener('change', function() {
            document.getElementById('use_common_ada').value = '1';
            commonAdaSection.style.display = 'block';
        });

        useMultipleAdaRadio.addEventListener('change', function() {
            document.getElementById('use_common_ada').value = '0';
            commonAdaSection.style.display = 'none';
        });
    }

    // Form validation and submission
    if (batchAdaForm) {
        batchAdaForm.addEventListener('submit', function(e) {
            // Check if any DVs are selected
            const selectedDVs = document.querySelectorAll('input[name="selected_dvs[]"]:checked');
            const selectedMergedGroups = document.querySelectorAll('input[name="selected_merged_groups[]"]:checked');
            
            if (selectedDVs.length === 0 && selectedMergedGroups.length === 0) {
                e.preventDefault();
                alert('Please select at least one disbursement voucher or merged group.');
                return;
            }
            ...
            if (useCommonAdaRadio.checked && !commonAdaRefInput.value) {
                e.preventDefault();
    ...
    // Update selected DVs count and total amount
    function updateSelectedDVs() {
        const selectedDVs = document.querySelectorAll('input[name="selected_dvs[]"]:checked');
        const selectedMergedGroups = document.querySelectorAll('input[name="selected_merged_groups[]"]:checked');
        const totalCount = selectedDVs.length + selectedMergedGroups.length;
        
        const selectedDVCount = document.getElementById('selectedDVCount');
        const selectedDVsBody = document.getElementById('selectedDVsBody');
        const totalSelectedAmount = document.getElementById('totalSelectedAmount');
        
        if (selectedDVCount) {
            selectedDVCount.textContent = totalCount + ' selected';
        }
        ...
        if (selectedDVsBody) {
            ...
            // Add selected DVs to the table
            selectedDVs.forEach(dv => {
                const dvId = dv.value;
                const dvRow = document.querySelector(`tr[data-dv-id="${dvId}"]`);
                if (dvRow) {
                    const amount = parseFloat(dvRow.getAttribute('data-amount'));
                    totalAmount += amount;
                    
                    const newRow = document.createElement('tr');
                    newRow.innerHTML = `
                        <td>${dvRow.querySelector('td:nth-child(1)').textContent}</td>
                        <td>${dvRow.querySelector('td:nth-child(2)').textContent}</td>
                        <td>₱${amount.toFixed(2)}</td>
                        <td>${useCommonAdaRadio.checked ? commonAdaRefInput.value : ''}</td>
                    `;
                    selectedDVsBody.appendChild(newRow);
                }
            });
            
            // Add selected merged groups to the table
            selectedMergedGroups.forEach(group => {
                const groupId = group.value;
                const groupRow = document.querySelector(`tr[data-merge-id="${groupId}"]`);
                if (groupRow) {
                    const amount = parseFloat(groupRow.getAttribute('data-amount'));
                    totalAmount += amount;
                    
                    const newRow = document.createElement('tr');
                    newRow.innerHTML = `
                        <td>${groupRow.querySelector('td:nth-child(1)').textContent}</td>
                        <td>${groupRow.querySelector('td:nth-child(2)').textContent}</td>
                        <td>₱${amount.toFixed(2)}</td>
                        <td>${useCommonAdaRadio.checked ? commonAdaRefInput.value : ''}</td>
                    `;
                    selectedDVsBody.appendChild(newRow);
            ...
            if (totalSelectedAmount) {
                totalSelectedAmount.textContent = '₱' + totalAmount.toFixed(2);
            }
        }
    }

    // Listen for changes in DV selection
    document.addEventListener('change', function(e) {
        if (e.target.matches('input[name="selected_dvs[]"], input[name="selected_merged_groups[]"]')) {
            updateSelectedDVs();
        }
    });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Helper function to show alerts (using SweetAlert2 if available, fallback to regular alerts)
