...
<body>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <button type="button" class="btn-close" id="closeDvModal" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                    ...
                    <form action="" method="post" id="jevForm">
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" name="jev_no" id="jev_no" autocomplete="off">
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-6">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary" name="submit" form="jevForm">Print</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
                        ...
                        .then(data => {
                            ...
                            if (Math.abs(totalDebit - totalCredit) > 0.01) { // Use small threshold for floating point comparison
                                ...
                                balanceRow.innerHTML = `
                                    <td colspan="2" class="text-end fw-bold">OUT OF BALANCE</td>
                                    <td colspan="2" class="text-center fw-bold">${Math.abs(totalDebit - totalCredit).toFixed(2)}</td>
                                `;
                                tableBody.appendChild(balanceRow);
                            }

                            // Show modal using Bootstrap
                            const jevModal = new bootstrap.Modal(modal);
                            jevModal.show();
                            ...
                            setTimeout(() => {
                                generateJEVNumber(); // Generate JEV number if needed
                            }, 100);
                        })
            ...
            // Close modal using Bootstrap methods
    ...
    <script>
        function generateJEVNumber() {
            ...
            if (!fundClusterInput || !fundClusterInput.value) return;
            
            const date = new Date(dateInput.value);
            const year = date.getFullYear();
            const month = (date.getMonth() + 1).toString().padStart(2, '0');
            
            fetch('fetch_jev_no.php?year=' + year + '&month=' + month)
                .then(response => response.json())
                .then(data => {
                    if (data.jev_no) {
                        document.getElementById('jev_no').value = data.jev_no;
                    }
                })
                .catch(error => {
                    console.error('Error fetching JEV number:', error);
                });
        }
    </script>
