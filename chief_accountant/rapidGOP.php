...
if (isset($_POST['submit'])) {
    ...
    if (empty($account_id) || empty($cash_allotment)) {
        $error_message = "Please fill in all required fields";
    } else {
        $sql = "INSERT INTO draft_project (draft_id, account_id, cash_allotment, balances, created_at) 
                VALUES (?, ?, ?, ?, ?)";
        $stmt = $connection->prepare($sql);
        $balances = $cash_allotment; // Initial balance equals cash allotment
        $stmt->bind_param("iidss", $draft_id, $account_id, $cash_allotment, $balances, $created_at);
...
$select = mysqli_query(
    ...
    "SELECT dp.*,
     ...
     WHERE YEAR(dp.created_at) = $selected_year
     ORDER BY an.account_name ASC"
);

$query_account = "SELECT account_id, account_name, account_number, type FROM account_name ORDER BY account_name ASC";
...
$total_Cashallotment = mysqli_fetch_assoc($total_Cashallotment_result)['total_Cashallotment'];
...
$update_balances_query = "UPDATE draft_project dp 
                         SET dp.balances = dp.cash_allotment - (
                             ...
                             WHERE dh.account_id = dp.account_id
...
<head>
    ...
    <title>Rapid Management - Budget System</title>
                ...
                <div class="card summary-card">
                    <div class="card-body p-4">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                <h5 class="card-title">Total Cash Allotment (<?php echo $months[$selected_month]; ?> <?php echo $selected_year; ?>)</h5>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="d-flex align-items-center">
                                                ...
                                                <span class="fw-medium"><?php echo htmlspecialchars($row['account_name']); ?></span>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="badge badge-code blue-badge"><?php echo htmlspecialchars($row['account_number']); ?></span>
                                        </td>
                                        <td class="text-end amount-cell">₱<?php echo htmlspecialchars(number_format($row['cash_allotment'], 2)); ?></td>
            ...
            <div class="modal-header bg-primary text-white">
                ...
                <div class="modal-body">
                    <form method="post" id="addUserForm">
                        ...
                        <div class="mb-3">
                            <label for="account_id" class="form-label">Account Name <span class="text-danger">*</span></label>
                            <select class="form-select" name="account_id" id="account_id" required>
                                ...
                                data-code='" . htmlspecialchars($row['account_number']) . "'>"
                                        . htmlspecialchars($row['account_name']) . " - " . htmlspecialchars($row['type']) . ""
                                        . "</option>";
                                }
                                ?>
                            </select>
                        ...
                        <div class="mb-3">
                            <label for="allotment" class="form-label">Cash Allotment <span class="text-danger">*</span></label>
                            <div class="input-group">
